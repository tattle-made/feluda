...
def setup_tables():
    ...
    cur.execute("insert into lang_ids values (3,'mr')")
    cur.execute("insert into lang_ids values (4,'bn')")
...
def setup_aligned_tables():
    ...
    cur.execute("insert into lang_ids values (0,'en')")
    cur.execute("insert into lang_ids values (1,'hi')")
    cur.execute("insert into lang_ids values (2,'mr')")
...
def insert_aligned_wordvecs(lang):
    ...
    for line in tqdm(fin,total=n):
        ...
        try:
            cur.execute("insert into wordvecs values ('%s','%s','%s')" % (word, lang_ids[lang], vec))
        except:
            print(tokens[0])
            continue
