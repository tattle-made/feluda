...
class TestFeludaMultiOperatorIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not getattr(self.__class__, "setup_successful", False):
            self.skipTest(
                f"Setup was not successful: {getattr(self.__class__, 'setup_error', 'Unknown error')}"
            )
    ...
    def test_video_vector_generation(self):
        ...
        self.assertTrue(
            first_vec["is_avg"], "First vector should be the average vector"
        )
        ...
        for vec_data in video_vec_generator:
            self.assertFalse(
                vec_data["is_avg"], "Subsequent vectors should be I-frame vectors"
            )
            i_frame_vectors.append(vec_data["vid_vec"])

        # There should be at least one I-frame
        self.assertTrue(
            len(i_frame_vectors) > 0, "Should have at least one I-frame vector"
        )
    ...
    def test_video_vector_consistency(self):
        ...
        if not self.has_video_operator:
            self.fail("vid_vec_rep_clip operator not available")
    ...
    def test_kmeans_clustering(self):
        ...
        if not self.has_cluster_operator:
            self.fail("cluster_embeddings operator not available")
        ...
        result = cluster_operator.run(
            self.sample_clustering_data, n_clusters=n_clusters, modality=modality
        )
        ...
        for key in result:
            self.assertTrue(
                key.startswith("cluster_"), "Cluster keys should start with 'cluster_'"
            )
            self.assertTrue(
                isinstance(result[key], list), "Cluster values should be lists"
            )
        ...
        self.assertEqual(
            len(all_items),
            len(self.sample_clustering_data),
    ...
    def test_agglomerative_clustering(self):
        ...
        for key, cluster_items in result.items():
            ...
            self.assertTrue(
                isinstance(cluster_items, list), "Cluster values should be lists"
            )
            all_items.extend(cluster_items)
    ...
    def test_video_to_clusters_integration(self):
        ...
        for i, vec in enumerate(i_frame_vectors):
            clustering_data.append(
                {"embedding": vec, "payload": {"id": f"iframe_{i}", "type": "iframe"}}
            )
    ...
    def test_full_pipeline(self):
        ...
        # 4. Verify results
        self.assertTrue(
            1 <= len(cluster_result) <= n_clusters,
            f"Should have between 1 and {n_clusters} clusters",
        )
