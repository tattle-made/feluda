...
class TestFeludaClassifyVideoZeroShotIntegration(unittest.TestCase):
    ...
    def test_operator_availability(self):
        ...
        self.assertIsNotNone(
            self.operator, "Operator should be available in the system"
        )
        self.assertTrue(
            hasattr(self.operator, "run"), "Operator should have a 'run' method"
        )
    ...
    def test_video_classification_structure(self):
        ...
        self.assertTrue(
            isinstance(result["prediction"], str), "Prediction should be a string"
        )
        self.assertIn(
            result["prediction"],
            self.sample_labels,
        ...
        self.assertAlmostEqual(
            sum(result["probs"]),
            1.0,
            places=len(self.sample_labels),
            msg="Probabilities should sum to approximately 1",
        )
        # Check each probability is a float between 0 and 1
        for prob in result["probs"]:
            self.assertTrue(
                isinstance(prob, float), "Each probability should be a float"
            )
            self.assertTrue(
                0 <= prob <= 1, "Each probability should be between 0 and 1"
            )
    ...
    def test_video_classification_results(self):
        ...
        self.assertEqual(
            result["prediction"], "cat", "Prediction should be 'cat' for a cat video"
        )

        # The first probability (for "cat") should be highest
        max_prob_index = np.argmax(result["probs"])
        self.assertEqual(
            max_prob_index, 0, "Highest probability should be for 'cat' (first label)"
        )
        ...
        self.assertTrue(
            all(cat_prob > p for p in other_probs),
    ...
    def test_different_labels(self):
        ...
        self.assertTrue(
            feline_prob > max(other_probs) or animal_prob > max(other_probs),
